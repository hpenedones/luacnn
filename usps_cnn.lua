...
function load_data_from_disk(folder)
	...
	for i=1,10 do
		...
		images_set[i] = gfx.loadPNG(folder .. filename,1)	-- images_set is global
		images_set[i]:resize(images_set[i]:size(1), images_set[i]:size(2), 1)
		if GUI_ON then
			windows[i] = gfx.Window() -- creates new window
			windows[i]:blit(images_set[i])
		end
	end
end
...
function get_example(class, example_id)
	...
	return image:sub( (example_col-1)*sub_image_width  + 1, example_col * sub_image_width , 	         	 						  (example_row-1)*sub_image_height + 1, example_row * sub_image_height  )
end
...
function test_predictor(predictor, test_dataset, classes, classes_names)
	...
	for i=1,test_dataset:size() do
		...
		local probabilites_per_class = lab.exp(responses_per_class)
		local probability, prediction = lab.max(probabilites_per_class) 
...
function main()
	...
	data_folder = arg[1] -- pass absolute path where data is from the command line
